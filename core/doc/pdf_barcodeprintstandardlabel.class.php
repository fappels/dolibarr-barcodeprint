 ...
 */
class pdf_barcodeprintstandardlabel extends CommonStickerGenerator
{
	/**
	 *	Constructor
	 *
	...
	{
		$this->_Line_Height = 8; // For big product ref and label, we use smalle font and so smaller line height
	 ...
	 *
	 * @param	TCPDF		$pdf			PDF reference
	 * @param	string		$textleft		Text center
	 ...
	 * @param	int			$imgscalewidth	Scale of image for width (1=Full width of sticker)
	 * @param	int			$imgscaleheight	Scale of image for height (1=Full height of sticker)
	 * @return	void
	 */
	public function Add_PDF_label(&$pdf, $textleft, $header, $footer, $outputlangs, $textright = '', $photo = '', $imgscalewidth = 1, $imgscaleheight = 1)
	{
		// phpcs:enable
		global $mysoc, $conf, $langs;
		global $forceimgscalewidth, $forceimgscaleheight;

		$imgscalewidth = (empty($forceimgscalewidth) ? $imgscalewidth : $forceimgscalewidth); // Scale of image for width (1=Full width of sticker)
		$imgscaleheight = (empty($forceimgscaleheight) ? $imgscaleheight : $forceimgscaleheight); // Scale of image for height (1=Full height of sticker)
		...
		} else // Only a right part CENTERED !!
		{
			// Output right area
			if ($textright == '%LOGO%' && $logo) {
				$pdf->Image($logo, $_PosX + ($this->_Width - $widthtouse) / 2, $_PosY + $ytop, $widthtouse, $heighttouse);
			} elseif ($textright == '%PHOTO%' && $photo) {
				$pdf->Image($photo, $_PosX + ($this->_Width - $widthtouse) / 2, $_PosY + $ytop, $widthtouse, $heighttouse);
			} else {
				$pdf->SetXY($_PosX + $xleft, $_PosY + $ytop);
		...
		if ($footer != '') {
			...
			$pdf->SetXY($_PosX, $_PosY + $this->_Height - $this->_Line_Height - 6);
			$pdf->MultiCell($this->_Width, $this->_Line_Height, $outputlangs->convToOutputCharset($footer), 0, 'C');
		}
	...
	public function write_file($arrayofrecords, $outputlangs, $srctemplatepath, $outputdir = '', $filename = 'tmp_address_sheet.pdf')
	{
		...
		$dir = (empty($outputdir) ? $conf->barcodeprint->dir_temp : $outputdir);
		$file = $dir."/".$filename;
		...
		foreach ($arrayofrecords as $val) {
			...
			$this->Add_PDF_label($pdf, $val['textleft'], $val['textheader'], $val['textfooter'], $langs, $val['textright'], $val['photo'], $val['imgscalewidth'], $val['imgscaleheight']);
		}
