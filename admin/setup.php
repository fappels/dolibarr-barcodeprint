...
$arrayofparameters = array(
	'BARCODEPRINT_DEFAULT_MODELLABEL'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
...
if ($useFormSetup && (float) DOL_VERSION >= 15) {
	...
	// Setup conf BARCODEPRINT_MYPARAM1 as a simple string input
	$item = $formSetup->newItem('BARCODEPRINT_MYPARAM1');

	// Setup conf BARCODEPRINT_MYPARAM1 as a simple textarea input but we replace the text of field title
	$item = $formSetup->newItem('BARCODEPRINT_MYPARAM2');
	$item->nameText = $item->getNameText().' more html text ';

	// Setup conf BARCODEPRINT_MYPARAM3
	$item = $formSetup->newItem('BARCODEPRINT_MYPARAM3');
	$item->setAsThirdpartyType();

	// Setup conf BARCODEPRINT_MYPARAM4 : exemple of quick define write style
	$formSetup->newItem('BARCODEPRINT_MYPARAM4')->setAsYesNo();

	// Setup conf BARCODEPRINT_MYPARAM5
	$formSetup->newItem('BARCODEPRINT_MYPARAM5')->setAsEmailTemplate('thirdparty');

	// Setup conf BARCODEPRINT_MYPARAM6
	$formSetup->newItem('BARCODEPRINT_MYPARAM6')->setAsSecureKey()->enabled = 0; // disabled

	// Setup conf BARCODEPRINT_MYPARAM7
	$formSetup->newItem('BARCODEPRINT_MYPARAM7')->setAsProduct();
	*/
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/barcodeprint/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'BARCODEPRINT_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'BARCODEPRINT_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'BARCODEPRINT_'.strtoupper($myTmpObjectKey).'_ADDON';
